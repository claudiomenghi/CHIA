 ...
 * which is accepting for the claim. 
 * </p>
 * 
 * <p>
 ...
 * is obtained from an accepting state of the claim, the perform method returns
 * true.
 * </p>
 * 
...
public class AcceptingClaimStatePathChecker {
	 ...
	 *            the set of the interesting source states
	 * @param interestingDestinationStates
	 *            the set interesting destinations states
	...
	public AcceptingClaimStatePathChecker(Set<State> allowedStates,
			...
			Set<State> interestingSourceStates,
		...
		statePresencePathChecker = new StatePresencePathChecker(
				...
				interestingStates);
		statePresencePathChecker.perform(interestingSourceStates,
				interestingDestinationStates);
	}

	/**
	 * returns true if the destination state, is reachable from the source
	 * state, through a path that involves only allowed states and crosses at
	 * least a state which is obtained from an accepting state of the claim.
	 * 
	 * @param source
	 *            the source state of interest
	 * @param destination
	 *            the destination state of interest
	 * @return true if the destination state is reachable from the source state,
	 *         through a path that involves only allowed states and crosses at
	 *         least a state which is obtained from an accepting state of the
	 *         claim.
	 * @throws NullPointerException
