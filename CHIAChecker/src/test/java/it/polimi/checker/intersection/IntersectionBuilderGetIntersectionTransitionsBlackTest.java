...
public class IntersectionBuilderGetIntersectionTransitionsBlackTest {
	...
	public void testUpdateIntersection() {
		...
		assertEquals(2, intersectionBuilder.getIntersectionTransitionsBlackBoxStatesMap().keySet().size());
		
		assertTrue(intersectionBuilder.getIntersectionTransitionsBlackBoxStatesMap().values().size()==2);
		Iterator<State> it=intersectionBuilder.getIntersectionTransitionsBlackBoxStatesMap().values().iterator();
		assertEquals(modelState2, it.next());
		assertEquals(modelState2, it.next());
	}
